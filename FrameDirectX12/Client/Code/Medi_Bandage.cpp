...
CMedi_Bandage::CMedi_Bandage(const CMedi_Bandage& rhs)
	: Engine::CGameObject(rhs)
	, m_tMeshInfo(rhs.m_tMeshInfo)
{
}

CMedi_Bandage::~CMedi_Bandage()
{
}

HRESULT CMedi_Bandage::Ready_GameObjectPrototype()
{

	return S_OK;
}

HRESULT CMedi_Bandage::Ready_GameObject()
{
...
HRESULT CMedi_Bandage::LateInit_GameObject()
{
...
_int CMedi_Bandage::Update_GameObject(const _float& fTimeDelta)
{
...
void CMedi_Bandage::Render_ShadowDepth(CShader_Shadow* pShader)
{
...
HRESULT CMedi_Bandage::Add_Component()
{
	NULL_CHECK_RETURN(m_pComponentMgr, E_FAIL);
...
void CMedi_Bandage::Set_ShadowTable(CShader_Shadow* pShader)
{
	_matrix matView = INIT_MATRIX;
	_matrix matProj = INIT_MATRIX;
...
CGameObject* CMedi_Bandage::Clone_GameObject(void* prg)
{
	CGameObject* pInstance = new CMedi_Bandage(*this);

	MeshInfo tMeshInfo = *reinterpret_cast<MeshInfo*>(prg);
	static_cast<CMedi_Bandage*>(pInstance)->m_tMeshInfo = tMeshInfo;
...
CMedi_Bandage* CMedi_Bandage::Create(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList)
{
	CMedi_Bandage* pInstance = new CMedi_Bandage(pGraphicDevice, pCommandList);
