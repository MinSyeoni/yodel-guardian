...
class ENGINE_DLL CDownSampleTarget : public CBase
{
private:
	explicit CDownSampleTarget(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
	virtual ~CDownSampleTarget() = default;

public:
	HRESULT Ready_Target();

	vector<ComPtr<ID3D12Resource>> GetTargetTexture() {
		return m_vecSampleTarget;
	};
...
private:
	...
	vector<ComPtr<ID3D12Resource>> m_vecSampleTarget;

	CRcTex* m_pBuffer;
	CShader_DefaultTex* m_pShader;

public:
	static CDownSampleTarget* Create(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
	virtual void Free();
};
