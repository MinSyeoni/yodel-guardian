...
CDownSampleTarget::CDownSampleTarget(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList)
	:m_pGraphicDev(pGraphicDevice), m_pCommandLst(pCommandList)
{
}

HRESULT CDownSampleTarget::Ready_Target()
{

	ComPtr<ID3D12Resource> pTex = nullptr;
	m_vecSampleTarget.push_back(pTex);
...
HRESULT CDownSampleTarget::SetUp_OnGraphicDev()
{
...
HRESULT CDownSampleTarget::Release_OnGraphicDev()
{
...
HRESULT CDownSampleTarget::BuildResource()
{
	...
	ThrowIfFailed(m_pGraphicDev->CreateCommittedResource(
		...
		D3D12_RESOURCE_STATE_RENDER_TARGET,
		&optClear,
		IID_PPV_ARGS(&m_vecSampleTarget[0])));
	...
	m_pGraphicDev->CreateRenderTargetView(m_vecSampleTarget[0].Get(), &desc,
...
void CDownSampleTarget::Render_RenderTarget()
{
...
void CDownSampleTarget::SetUp_ConstateTable()
{
...
HRESULT CDownSampleTarget::BuildDescriptors()
{
...
void CDownSampleTarget::OnResize(_uint newWidth, _uint newHeight)
{
}

CDownSampleTarget * CDownSampleTarget::Create(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList)
{
	CDownSampleTarget* pInstance = new CDownSampleTarget(pGraphicDevice, pCommandList);
