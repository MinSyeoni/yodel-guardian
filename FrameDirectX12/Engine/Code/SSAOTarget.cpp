...
CSSAOTarget::CSSAOTarget(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList)
	:m_pGraphicDev(pGraphicDevice), m_pCommandLst(pCommandList)
{
}

HRESULT CSSAOTarget::Ready_Target()
{

	ComPtr<ID3D12Resource> pTex = nullptr;
	m_vecSSAOTarget.push_back(pTex);
	...
	m_pShader->Set_Shader_Texture(m_vecSSAOTarget, (_uint)m_vecSSAOTarget.size());
...
HRESULT CSSAOTarget::SetUp_OnGraphicDev()
{
	...
	m_pCommandLst->ResourceBarrier(1,
		&CD3DX12_RESOURCE_BARRIER::Transition(m_vecSSAOTarget[0].Get(),
...
HRESULT CSSAOTarget::Release_OnGraphicDev()
{
...
HRESULT CSSAOTarget::BuildResource()
{
	...
	ThrowIfFailed(m_pGraphicDev->CreateCommittedResource(
		...
		D3D12_RESOURCE_STATE_RENDER_TARGET,
		&optClear,
		IID_PPV_ARGS(&m_vecSSAOTarget[0])));
	...
	m_pGraphicDev->CreateRenderTargetView(m_vecSSAOTarget[0].Get(), &desc,
...
void CSSAOTarget::Render_RenderTarget()
{
...
void CSSAOTarget::SetUp_ConstateTable()
{
	...
	matWorld._22 = 60.f;
	matWorld._33 = 1.f;
	matWorld._41 = (80 + 800) - WINSIZEX * 0.5f;
...
HRESULT CSSAOTarget::BuildDescriptors()
{
...
void CSSAOTarget::OnResize(_uint newWidth, _uint newHeight)
{
}

CSSAOTarget * CSSAOTarget::Create(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList)
{
	CSSAOTarget* pInstance = new CSSAOTarget(pGraphicDevice, pCommandList);
