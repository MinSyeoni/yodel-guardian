...
HRESULT CGraphicDevice::Create_GraphicDevice(const _uint& iWidth, const _uint& iHeight)
{
    ...
    DXGI_ADAPTER_DESC Desc,Desc2;
    pd3dAdapter->GetDesc(&Desc);
    m_pFactory->EnumAdapters1(1, &pd3dAdapter);
    pd3dAdapter->GetDesc(&Desc2);

    if (Desc.DedicatedVideoMemory < Desc2.DedicatedVideoMemory)
        iIndex = 1;




    m_pFactory->EnumAdapters1(iIndex, &pd3dAdapter);
...
HRESULT CGraphicDevice::Create_Dissolve()
{
    ...
    ThrowIfFailed(m_pGraphicDevice->CreateRootSignature(
        ...
        serializedRootSig->GetBufferPointer(),
        serializedRootSig->GetBufferSize(),
        IID_PPV_ARGS(&m_arrSig[(UINT)ROOT_SIG_TYPE::INPUT_DISSOLVE])));
