...
CShader_SSAO::CShader_SSAO(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList)
	: CShader(pGraphicDevice, pCommandList)
{
}

CShader_SSAO::CShader_SSAO(const CShader_SSAO & rhs)
	: CShader(rhs)
{
...
HRESULT CShader_SSAO::Ready_Shader()
{
...
void CShader_SSAO::Begin_Shader()
{
	...
	m_pCommandList->SetGraphicsRootSignature(CGraphicDevice::Get_Instance()->GetLootSig((_uint)ROOT_SIG_TYPE::INPUT_SSAO));

}

void CShader_SSAO::End_Shader()
{



	CD3DX12_GPU_DESCRIPTOR_HANDLE Noraml(m_pCBV_DescriptorHeap->GetGPUDescriptorHandleForHeapStart());
	Noraml.Offset(0, CGraphicDevice::Get_Instance()->Get_CBV_SRV_UAV_DescriptorSize());
	m_pCommandList->SetGraphicsRootDescriptorTable(0, Noraml);

	CD3DX12_GPU_DESCRIPTOR_HANDLE Depth(m_pCBV_DescriptorHeap->GetGPUDescriptorHandleForHeapStart());
	Depth.Offset(1, CGraphicDevice::Get_Instance()->Get_CBV_SRV_UAV_DescriptorSize());
	m_pCommandList->SetGraphicsRootDescriptorTable(1, Depth);
}

void CShader_SSAO::Set_Shader_Texture(vector< ComPtr<ID3D12Resource>> pVecTexture)
{
	if (m_bIsInit == true)
		return;
...
HRESULT CShader_SSAO::Create_DescriptorHeaps()
{
...
HRESULT CShader_SSAO::Create_ConstantBufferView()
{
	return S_OK;
}


HRESULT CShader_SSAO::Create_PipelineState()
{
	vector<D3D12_INPUT_ELEMENT_DESC> vecInputLayout;

	m_pVS_ByteCode = Compile_Shader(L"../../Bin/Shader/Shader_SSAO.hlsl", nullptr, "VS_MAIN", "vs_5_1");
	m_pPS_ByteCode = Compile_Shader(L"../../Bin/Shader/Shader_SSAO.hlsl", nullptr, "PS_MAIN", "ps_5_1");
	...
	PipelineStateDesc.pRootSignature = CGraphicDevice::Get_Instance()->GetLootSig((_uint)ROOT_SIG_TYPE::INPUT_SSAO);
...
D3D12_RASTERIZER_DESC CShader_SSAO::Create_RasterizerState()
{
...
D3D12_BLEND_DESC CShader_SSAO::Create_BlendState()
{
...
D3D12_DEPTH_STENCIL_DESC CShader_SSAO::Create_DepthStencilState()
{
...
D3D12_INPUT_LAYOUT_DESC CShader_SSAO::Create_InputLayout()
{
	return D3D12_INPUT_LAYOUT_DESC();
}


CComponent * CShader_SSAO::Clone()
{
	return new CShader_SSAO(*this);
}

CShader_SSAO * CShader_SSAO::Create(ID3D12Device * pGraphicDevice,
	ID3D12GraphicsCommandList * pCommandList)
{
	CShader_SSAO* pInstance = new CShader_SSAO(pGraphicDevice, pCommandList);
