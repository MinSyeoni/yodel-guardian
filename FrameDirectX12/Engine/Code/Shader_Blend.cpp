...
CShader_Blend::CShader_Blend(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList)
	: CShader(pGraphicDevice, pCommandList)
{
}

CShader_Blend::CShader_Blend(const CShader_Blend & rhs)
	: CShader(rhs)
{
...
HRESULT CShader_Blend::Ready_Shader()
{
...
void CShader_Blend::Begin_Shader()
{
	...
	m_pCommandList->SetGraphicsRootSignature(CGraphicDevice::Get_Instance()->GetLootSig((_uint)ROOT_SIG_TYPE::INPUT_BLEND));

}

void CShader_Blend::End_Shader()
{
	...
	tex.Offset(1, CGraphicDevice::Get_Instance()->Get_CBV_SRV_UAV_DescriptorSize());
	m_pCommandList->SetGraphicsRootDescriptorTable(2, tex);


	tex.Offset(1, CGraphicDevice::Get_Instance()->Get_CBV_SRV_UAV_DescriptorSize());
	m_pCommandList->SetGraphicsRootDescriptorTable(3, tex);
...
HRESULT CShader_Blend::Create_DescriptorHeaps()
{
...
HRESULT CShader_Blend::Create_ConstantBufferView()
{
...
HRESULT CShader_Blend::Create_PipelineState()
{
	vector<D3D12_INPUT_ELEMENT_DESC> vecInputLayout;

	m_pVS_ByteCode = Compile_Shader(L"../../Bin/Shader/Shader_Blend.hlsl", nullptr, "VS_MAIN", "vs_5_1");
	m_pPS_ByteCode = Compile_Shader(L"../../Bin/Shader/Shader_Blend.hlsl", nullptr, "PS_MAIN", "ps_5_1");
	...
	PipelineStateDesc.pRootSignature = CGraphicDevice::Get_Instance()->GetLootSig((_uint)ROOT_SIG_TYPE::INPUT_BLEND);
...
D3D12_RASTERIZER_DESC CShader_Blend::Create_RasterizerState()
{
...
D3D12_BLEND_DESC CShader_Blend::Create_BlendState()
{
...
D3D12_DEPTH_STENCIL_DESC CShader_Blend::Create_DepthStencilState()
{
...
D3D12_INPUT_LAYOUT_DESC CShader_Blend::Create_InputLayout()
{
...
{
	return new CShader_Blend(*this);
}

CShader_Blend * CShader_Blend::Create(ID3D12Device * pGraphicDevice,
	ID3D12GraphicsCommandList * pCommandList)
{
	CShader_Blend* pInstance = new CShader_Blend(pGraphicDevice, pCommandList);
