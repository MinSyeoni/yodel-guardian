...
CDistortionTarget::CDistortionTarget(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList)
	:m_pGraphicDev(pGraphicDevice), m_pCommandLst(pCommandList)
{
}

HRESULT CDistortionTarget::Ready_Target()
{

	ComPtr<ID3D12Resource> pTex = nullptr;
	//Shade¿ë
	m_vecDestortionTarget.push_back(pTex);
	...
	m_pShader->Set_Shader_Texture(m_vecDestortionTarget, (_uint)m_vecDestortionTarget.size());

	return S_OK;
}


HRESULT CDistortionTarget::SetUp_OnGraphicDev()
{
	CGraphicDevice::Get_Instance()->BackBufferSettingBegin();

	m_pCommandLst->ResourceBarrier(1,
		&CD3DX12_RESOURCE_BARRIER::Transition(m_vecDestortionTarget[0].Get(),
	...
	m_pCommandLst->OMSetRenderTargets(1, &CD3DX12_CPU_DESCRIPTOR_HANDLE(m_pRTV_Heap->GetCPUDescriptorHandleForHeapStart()), true, &CGraphicDevice::Get_Instance()->Get_DepthBuffer_handle());


	return S_OK;
}

HRESULT CDistortionTarget::Release_OnGraphicDev()
{
...
HRESULT CDistortionTarget::BuildResource()
{
	...
	for (int i = 0; i < 1; ++i)
	{
		...
		ThrowIfFailed(m_pGraphicDev->CreateCommittedResource(
			...
			D3D12_RESOURCE_STATE_RENDER_TARGET,
			&optClear,
			IID_PPV_ARGS(&m_vecDestortionTarget[i])));
	...
	for (int i = 0; i < 1; ++i)
	{
		m_pGraphicDev->CreateRenderTargetView(m_vecDestortionTarget[i].Get(), &desc,
...
void CDistortionTarget::Render_RenderTarget()
{
...
void CDistortionTarget::SetUp_ConstateTable(const int offset)
{
	...
	matWorld._42 = (-420) + WINSIZEY * 0.5f;
...
HRESULT CDistortionTarget::BuildDescriptors()
{
...
void CDistortionTarget::OnResize(_uint newWidth, _uint newHeight)
{
}

CDistortionTarget * CDistortionTarget::Create(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList)
{
	CDistortionTarget* pInstance = new CDistortionTarget(pGraphicDevice, pCommandList);
