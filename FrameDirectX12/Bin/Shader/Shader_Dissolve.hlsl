...
Texture2D gDissolveTexture : register(t4);
...
ps_output PS_MAIN(VS_OUTPUT input) : SV_TARGET
{
   ...
   ps_output output;
    ...
    float4 Dissolve = gDissolveTexture.Sample(gsamLinearWrap, input.uv);
    float1 fTime = matWorld._11;
    Dissolve.r = Dissolve.r + (fTime * 0.5f);
    saturate(Dissolve);

    output.albedo.a = Dissolve.r;
