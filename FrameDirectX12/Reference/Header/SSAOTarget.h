...
class ENGINE_DLL CSSAOTarget : public CBase
{
private:
	explicit CSSAOTarget(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
	virtual ~CSSAOTarget() = default;

public:
	HRESULT Ready_Target();

	vector<ComPtr<ID3D12Resource>> GetTargetTexture() {
		return m_vecSSAOTarget;
	};
...
private:
	...
	vector<ComPtr<ID3D12Resource>> m_vecSSAOTarget;

	CRcTex* m_pBuffer;
	CShader_DefaultTex* m_pShader;

public:
	static CSSAOTarget* Create(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
	virtual void Free();
};
