...
class ENGINE_DLL CLightTarget : public CBase
{
private:
	explicit CLightTarget(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
	virtual ~CLightTarget() = default;

public:
	HRESULT Ready_Target();

	vector<ComPtr<ID3D12Resource>> GetTargetTexture() {
		return m_veCLightTargetTexture;
	};
...
private:
	...
	vector<ComPtr<ID3D12Resource>> m_veCLightTargetTexture;
...
public:
	static CLightTarget* Create(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
	virtual void Free();
};
