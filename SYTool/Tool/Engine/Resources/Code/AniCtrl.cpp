...
void CAniCtrl::Set_AnimationTimeSet(const _uint & iIndex, float fTime)
{
	if (m_iOldAniIndex == iIndex)
		return;
	...
	// 현재 이 함수를 통해 재생되어지던 모션을 꺼버리는 함수 (3인자 : 언제부터 트랙에서 애니메이션을 해제할 것인가)
	// 이제부터 모든 실수는 double형을 사용
	m_pAniCtrl->KeyTrackEnable(m_iCurrentTrack, FALSE, m_fAccTime + fTime);

	// 애니메이션 재생 속도(위의 코드 근거로는 해제 속도를 설정하는 함수, 속도의 상수 값은 1)
	m_pAniCtrl->KeyTrackSpeed(m_iCurrentTrack, 1.f, m_fAccTime, fTime, D3DXTRANSITION_LINEAR);

	// 합성 시에 어떤 트랙에 있는 키 프레임에 가깝게 보간할 것인지 설정하는 함수(2인자 : 가중치 값, 절대 값은 1)
	m_pAniCtrl->KeyTrackWeight(m_iCurrentTrack, 0.1f, m_fAccTime, fTime, D3DXTRANSITION_LINEAR);


	m_pAniCtrl->SetTrackEnable(m_iNewTrack, TRUE);
	m_pAniCtrl->KeyTrackSpeed(m_iNewTrack, 1.f, m_fAccTime, fTime, D3DXTRANSITION_LINEAR);
	m_pAniCtrl->KeyTrackWeight(m_iNewTrack, 0.9f, m_fAccTime, fTime, D3DXTRANSITION_LINEAR);
...
_bool CAniCtrl::is_AnimationSetStart(void)
{
	D3DXTRACK_DESC			TrackInfo;
	ZeroMemory(&TrackInfo, sizeof(D3DXTRACK_DESC));
	...
	if (TrackInfo.Position+0.1f <= m_dPeriod)
		return true;

	return false;
}

void CAniCtrl::TimeReset(_double AniTime)
{
	D3DXTRACK_DESC			TrackInfo;
	ZeroMemory(&TrackInfo, sizeof(D3DXTRACK_DESC));
	...
	m_pAniCtrl->SetTrackPosition(m_iCurrentTrack, AniTime);
